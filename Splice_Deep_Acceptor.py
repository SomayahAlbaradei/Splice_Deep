...
def main(org='c_elegans', fname='test.fa'): 
    ...
    if "fname" in parameter_dict:
        print("Welcome to splice deep program : ") 
        print("organism is: " + parameter_dict["fname"])
    else: #Or if the user did not define var1 in their list:
    ...
    finalmodel='./models/acc_'+org+'.pkl'
    ...
    file = open("splicedeep_AcSS_output.txt","w")
    file.write(output)
    file.close()
    
    print('see prediction in splicedeep_AcSS_output.txt file')
